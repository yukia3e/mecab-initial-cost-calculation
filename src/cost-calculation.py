...
def run(csv_file_path, output_file_path):
  rows = []
  ...
  for row in rows:
    ...
    real_node = wakati.parseToNode(word)
    ...
    wakati = MeCab.Tagger(
        "-p -d /usr/lib/x86_64-linux-gnu/mecab/dic/mecab-ipadic-neologd")
    partial_text = convert_to_partial_format(word)
    partial_node = wakati.parseToNode(partial_text)
